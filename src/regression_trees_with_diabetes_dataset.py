...
# @Last Modified time: 2024-05-12 23:32:48
...
for depth in range(1, 6):  # Adjust the range if you want to evaluate more trees
    ...
    graph.render(
        f"{base_path}/img/diabetes_tree_depth_{depth}",
        format="png",
        view=True,
    )
...
tree_metrics.to_excel(
    f"{base_path}/test/tree_metrics_diabetes.xlsx",
    index=False,
)
print(f"Metrics saved to {base_path}/test/tree_metrics_diabetes.xlsx")
print(
...
graph = Source(dot_data)
graph.render(
    f"{base_path}/img/diabetes_best_tree_depth_{best_depth}",
    format='png',
    view=True,
)
